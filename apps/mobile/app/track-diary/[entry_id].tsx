...
import { TrackEntryMenu } from '../../components/track/TrackEntryMenu';
...
import { getTrackDiaryEntry, getTrackEntryComments, type TrackDiaryEntryDetail } from '../../lib/trackDiary';
import { toggleTrackDiaryLike, addTrackComment, deleteTrackComment } from '../../lib/feed';
...
export default function TrackDiaryEntryPage() {
  ...
  const [entry, setEntry] = useState<TrackDiaryEntryDetail | null>(null);
  ...
  const load = useCallback(async () => {
    if (!entry_id) return;
    const result = await getTrackDiaryEntry(entry_id);
    if (!result.success) {
      setNotFound(true);
      setLoading(false);
      return;
  ...
  const refreshComments = useCallback(async () => {
    if (!entry_id) return;
    const comments = await getTrackEntryComments(entry_id);
  ...
  const handleShare = async () => {
    if (!entry) return;
    try {
      await Share.share({ url: `${SITE_URL}/track-diary/${entry.id}`, message: `${SITE_URL}/track-diary/${entry.id}` });
    } catch {}
  };

  const handleCopyLink = async () => {
    if (!entry) return;
    await Clipboard.setStringAsync(`${SITE_URL}/track-diary/${entry.id}`);
        ...
        <View className="px-4 pt-4">
          ...
          <View className="mt-4 pb-5 border-b border-border flex-row gap-4 items-end">
            ...
            <View className="flex-1 pb-1">
              <Text className="text-text-tertiary mb-1" style={[labelStyle, { fontSize: 9.5 }]}>TITRE</Text>
              <Pressable onPress={() => router.push(`/tracks/${entry.track.id}` as any)}>
              ...
              <View className="flex-row items-baseline flex-wrap gap-2 mt-1.5">
                ...
                <Text
                  ...
                  style={[labelStyle, { fontSize: 12, textTransform: 'none', letterSpacing: 0 }]}
                  onPress={() => router.push(`/albums/${entry.album.id}` as any)}
                >
                  · {entry.album.title}
                </Text>
          ...
          {/* ── CTA ────────────────────────────────────────────────── */}
          <Pressable
            onPress={() => router.push(`/tracks/${entry.track.id}?scrollTo=reviews` as any)}
          ...
          >
            <View style={{ gap: 2 }}>
              ...
              <Text className="text-text-warm" style={{ fontFamily: 'InstrumentSerif_400Regular', fontSize: 16 }}>
                Les <Text style={{ fontFamily: 'InstrumentSerif_400Regular_Italic', color: '#5C4538' }}>critiques</Text> de ce titre
              </Text>
            </View>
