...
export function FeedCardReviewCreated({ event, currentUserId }: Props) {
    ...
    <Text>
      {isMe ? (
        ...
        <>
          ...
          <Text>{hasWords ? ' a écrit une critique' : ' a noté un album'}</Text>
        </>
      )}
    </Text>
  );
  ...
  const row = (
    <Pressable onPress={() => router.push(entryHref as any)} className="flex-row items-center gap-3">
      ...
      <FeedTextLines context={context} title={title} artist={artistOrExcerpt} time={hasWords ? '' : timeAgo} />
      <FeedRightCluster rating={event.rating} coverUrl={event.album?.cover_url} />
    </Pressable>
  );
