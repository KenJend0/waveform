...
import { getTrackReviewsPage, type TrackReview, type TrackReviewsTab } from '../../lib/trackDiary';
...
function ReviewItem({ review }: { review: TrackReview }) {
  ...
  return (
    <Pressable onPress={() => router.push(`/track-diary/${review.id}` as any)} className="bg-background-secondary rounded-card p-4">
      <View className="flex-row items-start gap-3">
        <Avatar src={review.avatar_url} size={28} />
        <View className="flex-1">
          <View className="flex-row items-baseline justify-between mb-1.5">
            <View className="flex-row items-baseline gap-1">
              <Text className="text-text-primary" style={metaMediumStyle}>{review.username || 'User'}</Text>
            ...
            {review.rating !== null && (
              <View className="flex-row items-baseline gap-0.5 bg-[#FAF8F4] border border-accent rounded-badge px-1.5 py-0.5">
                ...
                <Text className="uppercase text-accent" style={{ fontFamily: 'Inter_400Regular', fontSize: 9, opacity: 0.7 }}>/10</Text>
              </View>
            )}
          </View>
          {review.review_body && (
            <Text numberOfLines={3} className="text-accent-deep" style={{ fontFamily: 'InstrumentSerif_400Regular_Italic', fontSize: 13, lineHeight: 22.75 }}>
              «{' '}{review.review_body}{' '}»
            </Text>
          )}
        </View>
...
/** Miroir de TrackReviewSection + TrackReviewsBottomSheet (web) — voir ReviewsSection (mobile, albums) pour le pattern. */
export function TrackReviewsSection({ trackId, reviewsCount, initialReviews }: Props) {
  const [sheetOpen, setSheetOpen] = useState(false);
  const [items, setItems] = useState<TrackReview[]>([]);
  const [loading, setLoading] = useState(false);
  const [hasMore, setHasMore] = useState(false);
  const [offset, setOffset] = useState(0);
  const [tab, setTab] = useState<TrackReviewsTab>('all');
  ...
  async function loadReviews(startOffset: number, replace: boolean) {
    setLoading(true);
    try {
      const result = await getTrackReviewsPage({ trackId, tab, offset: startOffset, limit: 12 });
    ...
    <View className="border-t border-border-divider pt-8 mb-12">
      ...
      <BottomSheet isOpen={sheetOpen} onClose={() => setSheetOpen(false)} title="Critiques" snapPoint="70%">
        <View className="flex-row border-b border-border-divider px-6">
          {(['all', 'friends', 'my'] as TrackReviewsTab[])
            ...
            .map((t) => (
              ...
              >
                <Text className={tab === t ? 'text-text-primary' : 'text-text-secondary'} style={metaMediumStyle}>
        ...
        <View className="px-6 py-4" style={{ gap: 10 }}>
          {loading && items.length === 0 ? (
            ...
            <Text className="text-text-tertiary text-center py-8" style={metaStyle}>
              {tab === 'my' ? "Vous n'avez pas encore critiqué ce titre" : tab === 'friends' ? "Vos amis n'ont pas encore critiqué ce titre" : 'Pas de critiques pour l\'instant'}
            </Text>
          ) : (
            <>
